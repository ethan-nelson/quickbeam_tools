...
def read(filename, **kwargs):
    ...
    **kwargs: include only the variable of interest (tba)
    ...
    ref['hgt'] = np.reshape(struct.unpack(str(grid1d)+"f",
                            file.read(4*grid1d)), shape1d)
    ref['lat'] = np.reshape(struct.unpack(str(grid2d)+"f",
                            file.read(4*grid2d)), shape2d)
    ref['lon'] = np.reshape(struct.unpack(str(grid2d)+"f",
                            file.read(4*grid2d)), shape2d)
    ref['sfcrain'] = np.reshape(struct.unpack(str(grid2d)+"f",
                                file.read(4*grid2d)), shape2d)
    ref['tempk'] = np.reshape(struct.unpack(str(grid3d)+"f",
                              file.read(4*grid3d)), shape3d)
    ref['Z_eff'] = np.reshape(struct.unpack(str(grid3d)+"f",
                              file.read(4*grid3d)), shape3d)
    ref['Z_ray'] = np.reshape(struct.unpack(str(grid3d)+"f",
                              file.read(4*grid3d)), shape3d)
    ref['h_atten'] = np.reshape(struct.unpack(str(grid3d)+"f",
                                file.read(4*grid3d)), shape3d)
    ref['g_atten'] = np.reshape(struct.unpack(str(grid3d)+"f",
                                file.read(4*grid3d)), shape3d)
    ref['Z_cor'] = np.reshape(struct.unpack(str(grid3d)+"f",
                              file.read(4*grid3d)), shape3d)
